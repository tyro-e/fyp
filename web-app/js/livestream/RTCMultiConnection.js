...
window.RTCMultiConnection = function(roomid, forceOptions) {
        ...
        connection.peersBackup = {}, connection.peers = {
            ...
            return userPreferences || (userPreferences = {}), {
                ...
                onRemoteStream: function(stream) {
                    if (connection.peers[remoteUserId] && connection.peers[remoteUserId].streams.push(stream), isPluginRTC && window.PluginRTC) {
                        var mediaElement = document.createElement("video"),
                            body = connection.videosContainer;
                        return body.insertBefore(mediaElement, body.firstChild), void setTimeout(function() {
            ...
            if (!connection.peers[remoteUserId]) return void(connection.enableLogs && console.error("This peer (" + remoteUserId + ") does not exists. Renegotiation skipped."));
            ...
            if (!connection.peers[remoteUserId]) throw "This peer (" + remoteUserId + ") does not exists.";
    ...
    function getRMCMediaElement(stream, callback, connection) {
        ...
        if (isPluginRTC && window.PluginRTC) return connection.videosContainer.insertBefore(mediaElement, connection.videosContainer.firstChild), void setTimeout(function() {
            window.PluginRTC.attachMediaStream(mediaElement, stream), callback(mediaElement)
        }, 1e3);
        if (mediaElement[isFirefox ? "mozSrcObject" : "src"] = isFirefox ? stream : window.URL.createObjectURL(stream), mediaElement.controls = !0, isFirefox) {
            var streamEndedEvent = "ended";
            "oninactive" in stream && (streamEndedEvent = "inactive"), mediaElement.addEventListener(streamEndedEvent, function() {
                if (currentUserMediaRequest.remove(stream.idInstance), "local" === stream.type) {
                    StreamsHandler.onSyncNeeded(stream.streamid, streamEndedEvent), connection.attachStreams.forEach(function(aStream, idx) {
        ...
        mediaElement.play(), callback(mediaElement)
    }
    ...
    function isAudioPlusTab(connection, audioPlusTab) {
        return (!connection.session.audio || "two-way" !== connection.session.audio) && (!(!isFirefox || audioPlusTab === !1) || !(!isChrome || chromeVersion < 50) && (typeof audioPlusTab === !0 || !("undefined" != typeof audioPlusTab || !connection.session.audio || !connection.session.screen || connection.session.video) && (audioPlusTab = !0, !0)))
    }
    ...
    function PeerInitiator(config) {
        ...
        function createOfferOrAnswer(_method) {
            peer[_method](function(localSdp) {
                localSdp.sdp = connection.processSdp(localSdp.sdp), peer.setLocalDescription(localSdp, function() {
        ...
        if (!RTCPeerConnection) throw "WebRTC 1.0 (RTCPeerConnection) API are NOT available in this browser.";
        ...
        if (connection.attachStreams.forEach(function(stream) {
            ...
            (connection.candidates.turn || connection.candidates.relay) && (connection.candidates.stun || connection.candidates.reflexive || connection.candidates.host || (iceTransports = "relay")), peer = new RTCPeerConnection(navigator.onLine ? {
                iceServers: connection.iceServers,
                iceTransportPolicy: connection.iceTransportPolicy || iceTransports,
                rtcpMuxPolicy: connection.rtcpMuxPolicy || "negotiate"
            } : null, window.PluginRTC ? null : connection.optionalArgument), connection.iceServers.length || (peer = new RTCPeerConnection(null, null))
        }
        ...
        config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.isFirefoxOffered && (isFirefoxOffered = !0), localStreams.forEach(function(localStream) {
            config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.dontGetRemoteStream || config.dontAttachLocalStream || (localStream = connection.beforeAddingStream(localStream), localStream && (getLocalStreams().forEach && getLocalStreams().forEach(function(stream) {
        ...
        var streamObject, remoteStreamAddEvent = "addstream";
        peer.addEventListener(remoteStreamAddEvent, function(event) {
            if (event) {
                if (event.streams && event.streams.length && !event.stream) {
                    if (!streamObject) return void(streamObject = new MediaStream);
                    if (event.streams.forEach(function(stream) {
                            stream.getVideoTracks().length && streamObject.addTrack(stream.getVideoTracks()[0]), stream.getAudioTracks().length && streamObject.addTrack(stream.getAudioTracks()[0])
                        }), event.stream = streamObject, connection.session.audio && connection.session.video && (!streamObject.getVideoTracks().length || !streamObject.getAudioTracks().length)) return;
                    streamObject = null
                }
                var streamsToShare = {};
                ...
                streamToShare && (event.stream.isAudio = streamToShare.isAudio, event.stream.isVideo = streamToShare.isVideo, event.stream.isScreen = streamToShare.isScreen), event.stream.streamid = event.stream.id, event.stream.stop || (event.stream.stop = function() {
                    if (isFirefox) {
                        var streamEndedEvent = "ended";
            ...
            remoteSdp.sdp = connection.processSdp(remoteSdp.sdp), peer.setRemoteDescription(new RTCSessionDescription(remoteSdp), cb || function() {}, function(error) {
    ...
    function loadIceFrame(callback, skip) {
        if (!loadedIceFrame) {
            if (!skip) return loadIceFrame(callback, !0);
            loadedIceFrame = !0;
            var iframe = document.createElement("iframe");
            iframe.onload = function() {
                function iFrameLoaderCallback(event) {
                    event.data && event.data.iceServers && (callback(event.data.iceServers), window.removeEventListener("message", iFrameLoaderCallback))
                }
                iframe.isLoaded = !0, listenEventHandler("message", iFrameLoaderCallback), iframe.contentWindow.postMessage("get-ice-servers", "*")
            }, iframe.src = "https://cdn.webrtc-experiment.com/getIceServers/", iframe.style.display = "none", (document.body || document.documentElement).appendChild(iframe)
        }
    }
    ...
    function getSTUNObj(stunStr) {
        ...
        var obj = {};
        return obj[urlsParam] = stunStr, obj
    }
    ...
    function getTURNObj(turnStr, username, credential) {
        ...
        return obj[urlsParam] = turnStr, obj
    }

    function getExtenralIceFormatted() {
        var iceServers = [];
        return window.RMCExternalIceServers.forEach(function(ice) {
            ice.urls || (ice.urls = ice.url), ice.urls.search("stun|stuns") !== -1 && iceServers.push(getSTUNObj(ice.urls)), ice.urls.search("turn|turns") !== -1 && iceServers.push(getTURNObj(ice.urls, ice.username, ice.credential))
        }), iceServers
    }
        ...
        if (currentUserMediaRequest.streams[idInstance]) streaming(currentUserMediaRequest.streams[idInstance].stream, !0);
            ...
            if (isPluginRTC && window.PluginRTC) {
                ...
                return void window.PluginRTC.getUserMedia({
                    ...
                    stream.streamid = stream.id || getRandomString(), streaming(stream)
                }, function(error) {})
            }
    ...
    function getChromeExtensionStatus(extensionid, callback) {
        ...
        if (2 != arguments.length && (callback = extensionid, extensionid = window.RMCExtensionID || "ajhifddimkapgcifgcodmmfdlknahffk"), isFirefox) return callback("not-chrome");
    ...
    function getScreenConstraints(callback, audioPlusTab) {
        ...
        return isFirefox ? callback(null, firefoxScreenConstraints) : (sourceId = null, void isChromeExtensionAvailable(function(isAvailable) {
    ...
    "undefined" != typeof cordova && (isMobileDevice = !0, isChrome = !0), navigator && navigator.userAgent && navigator.userAgent.indexOf("Crosswalk") !== -1 && (isMobileDevice = !0, isChrome = !0);
    var isPluginRTC = !isMobileDevice && (isSafari || isIE);
    isPluginRTC && "undefined" != typeof URL && (URL.createObjectURL = function() {});
    var chromeVersion = (!!(window.process && "object" == typeof window.process && window.process.versions && window.process.versions["node-webkit"]), 50),
    ...
    matchArray = navigator.userAgent.match(/Firefox\/(.*)/), isFirefox && matchArray && matchArray[1] && (firefoxVersion = parseInt(matchArray[1], 10)), window.addEventListener || (window.addEventListener = function(el, eventName, eventHandler) {
        ...
        function() {
            ...
            function getIPs(callback) {
                function handleCandidate(candidate) {
                    ...
                    if (!match) return void console.warn("Could not match IP address in", candidate);
                ...
                if (!RTCPeerConnection) {
                    ...
                    if (!iframe) throw "NOTE: you need to have an iframe in the page right above the script tag.";
                    var win = iframe.contentWindow;
                    RTCPeerConnection = win.RTCPeerConnection || win.mozRTCPeerConnection || win.webkitRTCPeerConnection, useWebKit = !!win.webkitRTCPeerConnection
                }
                if (RTCPeerConnection) {
            ...
            if (isMobile.any()) osName = isMobile.getOsName();
            else {
            ...
            ["captureStream", "mozCaptureStream", "webkitCaptureStream"].forEach(function(item) {
                !isCanvasSupportsStreamCapturing && item in document.createElement("canvas") && (isCanvasSupportsStreamCapturing = !0), !isVideoSupportsStreamCapturing && item in document.createElement("video") && (isVideoSupportsStreamCapturing = !0)
            });
            var MediaDevices = [],
                audioInputDevices = [],
                audioOutputDevices = [],
            ...
            var isWebRTCSupported = (!!(window.process && "object" == typeof window.process && window.process.versions && window.process.versions["node-webkit"]), !1);
            ...
            DetectRTC.browser.isChrome && DetectRTC.browser.version >= 35 ? isScreenCapturingSupported = !0 : DetectRTC.browser.isFirefox && DetectRTC.browser.version >= 34 && (isScreenCapturingSupported = !0), "https:" !== location.protocol && (isScreenCapturingSupported = !1), DetectRTC.isScreenCapturingSupported = isScreenCapturingSupported;
            var webAudio = {
                isSupported: !1,
            ...
            DetectRTC.displayResolution = displayResolution, DetectRTC.isCanvasSupportsStreamCapturing = isCanvasSupportsStreamCapturing, DetectRTC.isVideoSupportsStreamCapturing = isVideoSupportsStreamCapturing, "Chrome" == DetectRTC.browser.name && DetectRTC.browser.version >= 53 && (DetectRTC.isCanvasSupportsStreamCapturing || (DetectRTC.isCanvasSupportsStreamCapturing = "Requires chrome flag: enable-experimental-web-platform-features"), DetectRTC.isVideoSupportsStreamCapturing || (DetectRTC.isVideoSupportsStreamCapturing = "Requires chrome flag: enable-experimental-web-platform-features")), DetectRTC.DetectLocalIPAddress = DetectLocalIPAddress, DetectRTC.isWebSocketsSupported = "WebSocket" in window && 2 === window.WebSocket.CLOSING, DetectRTC.isWebSocketsBlocked = !DetectRTC.isWebSocketsSupported, DetectRTC.checkWebSocketsSupport = function(callback) {
            ...
            }, DetectRTC.MediaDevices = MediaDevices, DetectRTC.hasMicrophone = hasMicrophone, DetectRTC.hasSpeakers = hasSpeakers, DetectRTC.hasWebcam = hasWebcam, DetectRTC.isWebsiteHasWebcamPermissions = isWebsiteHasWebcamPermissions, DetectRTC.isWebsiteHasMicrophonePermissions = isWebsiteHasMicrophonePermissions, DetectRTC.audioInputDevices = audioInputDevices, DetectRTC.audioOutputDevices = audioOutputDevices, DetectRTC.videoInputDevices = videoInputDevices;
            var isSetSinkIdSupported = !1;
            "setSinkId" in document.createElement("video") && (isSetSinkIdSupported = !0), DetectRTC.isSetSinkIdSupported = isSetSinkIdSupported;
    ...
    window.onPluginRTCInitialized = function() {
        MediaStreamTrack = window.PluginRTC.MediaStreamTrack, RTCPeerConnection = window.PluginRTC.RTCPeerConnection, RTCIceCandidate = window.PluginRTC.RTCIceCandidate, RTCSessionDescription = window.PluginRTC.RTCSessionDescription
    ...
    var CodecsHandler = function() {
        ...
        function preferVP9(sdp) {
            return sdp.indexOf("SAVPF 100 101") === -1 || sdp.indexOf("VP9/90000") === -1 ? sdp : sdp.replace("SAVPF 100 101", "SAVPF 101 100")
        }
        ...
        return "undefined" != typeof cordova && (isMobileDevice = !0), navigator && navigator.userAgent && navigator.userAgent.indexOf("Crosswalk") !== -1 && (isMobileDevice = !0), {
            removeVPX: removeVPX,
        ...
        FileProgressBarHandler = function() {
            function handle(connection) {
                ...
                connection.onFileStart = function(file) {
                    ...
                    if (!helper) return void console.error("No such progress-helper element exists.", file);
    ...
    ! function(connection) {
        ...
        mPeer.onGettingLocalMedia = function(stream) {
            ...
            connection.userid;
            return connection.userid = connection.sessionid = localUserid || connection.sessionid, connection.userid += "", connection.isInitiator = !0, connectSocket(function() {
                connection.socket.emit("changed-uuid", connection.userid), 1 == isPublicModerator && connection.becomePublicModerator()
            ...
            connection.userid = newUserId || getRandomString(), connection.socket.emit("changed-uuid", connection.userid, callback || function() {})
            ...
            return isMobileDevice || isFirefox ? sdp : (sdp = CodecsHandler.setApplicationSpecificBandwidth(sdp, connection.bandwidth, !!connection.session.screen), sdp = CodecsHandler.setVideoBitrates(sdp, {
            ...
            if (DetectRTC.MediaDevices.forEach(function(device) {
                ...
                if (isFirefox) return void(connection.mediaConstraints.audio !== !0 ? connection.mediaConstraints.audio.deviceId = lastAudioDevice.id : connection.mediaConstraints.audio = {
            ...
            if (lastVideoDevice) {
                if (isFirefox) return void(connection.mediaConstraints.video !== !0 ? connection.mediaConstraints.video.deviceId = lastVideoDevice.id : connection.mediaConstraints.video = {
        ...
        }, connection.rtcpMuxPolicy = "negotiate", connection.iceTransportPolicy = null, connection.optionalArgument = {
            optional: [{
            ...
            parentNode.insertBefore(e.mediaElement, parentNode.firstChild), e.mediaElement.play(), setTimeout(function() {
                e.mediaElement.play()
            }, 5e3)
        ...
        }, connection.direction = "many-to-many", connection.removeStream = function(streamid) {
            var stream;
            ...
            }), connection.userid = sender, connection.socket.emit("changed-uuid", connection.userid)
            ...
            e && e.mediaElement && ("both" === e.muteType || "video" === e.muteType ? (e.mediaElement.src = null, e.mediaElement.pause(), e.mediaElement.poster = e.snapshot || "https://cdn.webrtc-experiment.com/images/muted.png") : "audio" === e.muteType && (e.mediaElement.muted = !0))
        }, connection.onunmute = function(e) {
            e && e.mediaElement && e.stream && ("both" === e.unmuteType || "video" === e.unmuteType ? (e.mediaElement.poster = null, e.mediaElement.src = URL.createObjectURL(e.stream), e.mediaElement.play()) : "audio" === e.unmuteType && (e.mediaElement.muted = !1))
        ...
        }, connection.trickleIce = !0, connection.version = "3.4.3", connection.onSettingLocalDescription = function(event) {
